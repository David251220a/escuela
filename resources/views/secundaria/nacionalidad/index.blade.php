                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                               ...
                               {{-- EL foreach se utiliza cuando tenes una consulta tipo get --}}
                                @foreach ($nacionalidad as $nacionalidad)
                                    <tr>
                                        <td class="px-6 py-2 whitespace-nowrap text-sm text-black font-semibold text-center">{{$nacionalidad->id }}</td>
                                        <td class="px-6 py-2 whitespace-nowrap text-sm text-black font-semibold text-left">{{ $nacionalidad->nombre }}</td>
                                        <td class="px-6 py-2 whitespace-nowrap text-sm text-black font-semibold text-center">{{ $nacionalidad->estado->nombre }}</td>
                                        <td class="px-6 py-2 whitespace-nowrap text-sm text-black font-semibold text-right">
                                        <a href="{{ route('nacionalidad.edit',$nacionalidad) }}" class="text-sm px-4 py-2 mb-4 border rounded  text-white font-bold" style="border-color: rgb(255, 136, 0); background : orange;"> Editar</a>
                                        </td>
                                    </tr>
                                @endforeach
