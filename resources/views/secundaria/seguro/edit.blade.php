...
<h2 class="text-gray-600 font-semibold text-xl mb-6" >EDITAR SEGURO  :      {{$seguro->nombre}}</h2>

    {{-- todo lo que esta ak adentro va a el controlador update --}}
    <form action="{{route('seguro.update', $seguro)}}" method="POST">
       @method('PUT')
       @csrf
      ...
      <div name="Datos_de_la_tabla_seguro">
       ...
       <div class="mb-4">
           ...
           <input type="text" class="block w-full border-gray-300 py-2 text-xl rounded" name="nombre" value = "{{$seguro->nombre}}" >
       </div>
       ...
       <div class="mb-4">
           ...
           <select name="estado_id" id="estado_id" class="block w-full border-gray-300 py-2 text-xl rounded">
               <option {{($seguro->estado_id==2 ? 'selected' :'')}} value="2">Inactivo</option>
               <option {{($seguro->estado_id==1 ? 'selected' :'')}} value="1">Activo</option>
           </select>
       </div>
     </div>
