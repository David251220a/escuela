...
<h2 class="text-gray-600 font-semibold text-xl mb-6" >EDITAR ENFERMEDAD  :      {{$enfermedad->nombre}}</h2>

    {{-- todo lo que esta ak adentro va a el controlador update --}}
    <form action="{{route('enfermedad.update', $enfermedad)}}" method="POST">
       @method('PUT')
       @csrf

      <div name="Datos_de_la_tabla_enfermedad">
       {{-- El Mb-4 es el espacio entre los campos --}}
       <div class="mb-4">
           <label for="" class="text-gray-500 mb-1 text-lg" >Nombre</label>
           <input type="text" class="block w-full border-gray-300 py-2 text-xl rounded" name="nombre" value = "{{$enfermedad->nombre}}" >
       </div>
       ...
       <div class="mb-4">
           ...
           <select name="estado_id" id="estado_id" class="block w-full border-gray-300 py-2 text-xl rounded">
               <option {{($enfermedad->estado_id==2 ? 'selected' :'')}} value="2">Inactivo</option>
               <option {{($enfermedad->estado_id==1 ? 'selected' :'')}} value="1">Activo</option>
           </select>
       </div>
     </div>
