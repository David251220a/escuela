<x-app-layout>
    ...
    <div class="mb-6">
        <a class="text-sm px-4 py-2 mb-4 border rounded  text-white font-bold" style="border-color: blue; background : rgb(7, 101, 189);"
         href="{{ route('enfermedad.create') }}">AGREGAR ENFERMEDAD</a>
    </div>

    {{-- @livewire('alergia-index') --}}
    <div class="mt-4">

        <h2 class="font-bold text-gray-500 text-2xl mb-4">LISTADO DE TODAS LAS ENFERMEDADES</h2>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                               ...
                               {{-- EL foreach se utiliza cuando tenes una consulta tipo get --}}
                                @foreach ($enfermedad as $enfermedad)
                                    <tr>
                                        <td class="px-6 py-2 whitespace-nowrap text-sm text-black font-semibold text-center">{{$enfermedad->id }}</td>
                                        <td class="px-6 py-2 whitespace-nowrap text-sm text-black font-semibold text-left">{{ $enfermedad->nombre }}</td>
                                        <td class="px-6 py-2 whitespace-nowrap text-sm text-black font-semibold text-center">{{ $enfermedad->estado->nombre }}</td>
                                        <td class="px-6 py-2 whitespace-nowrap text-sm text-black font-semibold text-right">
                                        <a href="{{ route('enfermedad.edit',$enfermedad) }}" class="text-sm px-4 py-2 mb-4 border rounded  text-white font-bold" style="border-color: rgb(255, 136, 0); background : orange;"> Editar</a>
                                        </td>
                                    </tr>
                                @endforeach
