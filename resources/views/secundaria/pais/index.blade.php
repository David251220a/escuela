<x-app-layout>
    ...
    <div class="mb-6">
        <a class="text-sm px-4 py-2 mb-4 border rounded  text-white font-bold" style="border-color: blue; background : rgb(7, 101, 189);"
         href="{{ route('pais.create') }}">AGREGAR PAIS</a>
    </div>

    {{-- @livewire('alergia-index') --}}
    <div class="mt-4">
        <h2 class="font-bold text-gray-500 text-2xl mb-4 text-center">LISTADO DE TODOS LOS PAISES</h2>

        <input type="hidden" name="nuevo" id="nuevo" value="{{session()->get('message')}}">
        {{-- <div class="mt-4 mb-4">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                               ...
                               {{-- EL foreach se utiliza cuando tenes una consulta tipo get --}}
                                @foreach ($pais as $item)
                                    <tr>
                                        <td class="px-6 py-2 whitespace-nowrap text-sm text-black font-semibold text-center">{{$item->id }}</td>
                                        <td class="px-6 py-2 whitespace-nowrap text-sm text-black font-semibold text-left">{{ $item->nombre }}</td>
                                        <td class="px-6 py-2 whitespace-nowrap text-sm text-black font-semibold text-center">{{ $item->estado->nombre }}</td>
                                        <td class="px-6 py-2 whitespace-nowrap text-sm text-black font-semibold text-right">
                                        <a href="{{ route('pais.edit',$item) }}" class="text-sm px-4 py-2 mb-4 border rounded  text-white font-bold" style="border-color: rgb(255, 136, 0); background : orange;"> Editar</a>
                                        </td>
                                    </tr>
                                @endforeach
        ...
        {{-- Cuando le damos al boton cancelar te envia de vuelta a la pantalla principla. --}}
    ...
    <script>
        window.onload = function () {
            ...
            if(nuevo != ''){
                Swal.fire(
                    'Nuevo Pais',
                    'El Pais se ha creado con exito',
                    'success'
                )
            }
